...
فرض کنید دامنه‌تون `pbp.com` هست. می‌تونید دامنه اصلی یا یه زیردامنه مثل `xyz.pbp.com` رو وارد کنید، بعد روی `Add domain` کلیک کنید.
...
بعدش می‌تونید از آدرس `https://xyz.pbp.com/panel` وارد پنلتون بشید و اشتراک‌های جدید بگیرید.
...
برای به‌روزرسانی پنل، فایل worker.js جدید رو از اینجا دانلود کنید. توی اکانت Cloudflare، از نوارابزار به `Compute & AI` > `Workers & Pages` برید، پروژه Worker رو انتخاب کنید، ویرایش کنید، فایل قدیمی رو حذف کنید، فایل جدید رو آپلود کنید و Deploy کنید.
