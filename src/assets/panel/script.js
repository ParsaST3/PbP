...
function exportSettings() {
    ...
    downloadJSON(encodedData, `PbP-settings.dat`);
}
...
(function initQuickSettings() {
    const regionSelect = document.getElementById('quickRegion');
    const fixedIPCheck = document.getElementById('quickFixedIP');
    const applyBtn = document.getElementById('quickApplyButton');
    const statusEl = document.getElementById('quickSettingsStatus');

    if (!regionSelect || !applyBtn) return;

    let regionMap = {};

    fetch('/proxy-ip/get', { credentials: 'same-origin' })
        .then(res => res.json())
        .then(({ success, body }) => {
            regionSelect.innerHTML = '';
            if (!success || !body || !body.length) {
                regionSelect.innerHTML = '<option value="">هیچ IP ای پیدا نشد</option>';
                return;
            }

            body.forEach(item => {
                if (!item.ip || !item.country) return;
                if (!regionMap[item.country]) {
                    regionMap[item.country] = item.ip;
                }
            });
            ...
            Object.keys(regionMap).sort().forEach(country => {
                const opt = document.createElement('option');
                opt.value = regionMap[country];
                opt.textContent = country;
                regionSelect.appendChild(opt);
            });
        })
        .catch(() => {
            regionSelect.innerHTML = '<option value="">خطا در دریافت لیست</option>';
        });

    // Sync quick fixed-IP toggle with initial real setting once panel data loads
    const syncInterval = setInterval(() => {
        const realFixedIP = document.getElementById('fixedProxyIP');
        if (realFixedIP) {
            fixedIPCheck.checked = realFixedIP.checked;
            clearInterval(syncInterval);
        }
    }, 200);

    applyBtn.addEventListener('click', () => {
        const realProxyIPMode = document.getElementById('proxyIPMode');
        const realProxyIPs = document.getElementById('proxyIPs');
        const realFixedIP = document.getElementById('fixedProxyIP');

        if (!realProxyIPMode || !realProxyIPs || !realFixedIP) {
            statusEl.textContent = '❌ خطا: فرم تنظیمات پیدا نشد.';
            return;
        }

        if (regionSelect.value) {
            realProxyIPMode.value = 'proxyip';
            realProxyIPMode.dispatchEvent(new Event('change', { bubbles: true }));
            realProxyIPs.value = regionSelect.value;
            realProxyIPs.dispatchEvent(new Event('input', { bubbles: true }));
        }

        realFixedIP.checked = fixedIPCheck.checked;
        realFixedIP.dispatchEvent(new Event('change', { bubbles: true }));

        statusEl.textContent = '⏳ در حال اعمال...';
...
(function initUsersManagement() {
    ...
    function formatExpiry(expiresAt) {
        ...
        return expired ? `${str} (منقضی شده)` : str;
    }

    function renderUsers(users) {
        if (!users || !users.length) {
            tbody.innerHTML = '<tr><td colspan="5">هنوز کاربری اضافه نشده.</td></tr>';
            return;
        }
        ...
        users.forEach(u => {
            ...
            tr.innerHTML = `
                ...
                <td style="padding:6px; direction:ltr; font-family:monospace; font-size:0.85em;">${u.uuid}</td>
                <td style="padding:6px; direction:ltr; font-family:monospace; font-size:0.85em;">${u.trPass}</td>
                <td style="padding:6px;">${formatExpiry(u.expiresAt)}</td>
                <td style="padding:6px;"><button type="button" class="button" data-delete-id="${u.id}">🗑️</button></td>
            `;
    ...
    function loadUsers() {
        fetch('/panel/users', { credentials: 'same-origin' })
            .then(res => res.json())
            .then(({ success, body }) => {
                if (success) renderUsers(body);
                else tbody.innerHTML = '<tr><td colspan="5">خطا در بارگذاری کاربران.</td></tr>';
            })
            .catch(() => {
                tbody.innerHTML = '<tr><td colspan="5">خطا در بارگذاری کاربران.</td></tr>';
            });
    }

    function deleteUser(id) {
        if (!confirm('این کاربر حذف بشه؟')) return;
        fetch('/panel/users/delete', {
            method: 'POST',
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
    ...
    addBtn.addEventListener('click', () => {
        ...
        if (!name) {
            statusEl.textContent = '❌ نام کاربر رو وارد کن.';
            return;
        }
        ...
        fetch('/panel/users/add', {
            ...
            .then(res => res.json())
            .then(({ success, body, message }) => {
                if (success) {
                    renderUsers(body);
                    nameInput.value = '';
                    expiryInput.value = '';
                    statusEl.textContent = '✅ کاربر اضافه شد.';
                } else {
                    statusEl.textContent = `❌ ${message || 'خطا در افزودن کاربر.'}`;
                }
            })
            .catch(() => {
                statusEl.textContent = '❌ خطا در افزودن کاربر.';
            });
    });
