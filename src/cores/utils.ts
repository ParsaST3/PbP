...
export function generateWsPath(protocol: string): string {
    const {
        settings: { proxyIPMode, proxyIPs, prefixes, fixedProxyIP },
        dict: { _VL_ }
    } = globalThis;

    const modeIPs = proxyIPMode === 'proxyip' ? proxyIPs : prefixes;
    const panelIPs = fixedProxyIP && modeIPs.length ? [modeIPs[0]] : modeIPs;
