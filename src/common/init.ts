...
export async function initWs(env: any) {
    ...
    try {
        const allUsers: ExtraUser[] = (await env.kv.get("extraUsers", { type: 'json' })) || [];
        const now = Date.now();
        globalThis.globalConfig.extraUsers = allUsers.filter(u =>
            !u.expiresAt || new Date(u.expiresAt).getTime() > now
        );
    } catch (error) {
        globalThis.globalConfig.extraUsers = [];
    }
}
