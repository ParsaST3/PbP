...
export async function handlePanel(request: Request, env: Env): Promise<Response> {
    ...
    switch (pathName) {
        ...
        case '/panel/users/add':
            return await addUser(request, env);

        case '/panel/users/delete':
...
async function getUsers(env: Env): Promise<Response> {
    try {
        const users: ExtraUser[] = (await env.kv.get("extraUsers", { type: 'json' })) || [];
        return respond(true, HttpStatus.OK, '', users);
    } catch (error) {
        console.log(error);
        return respond(false, HttpStatus.INTERNAL_SERVER_ERROR, `Error occurred while getting users: ${safeErrorMessage(error)}`);
    }
}
...
async function addUser(request: Request, env: Env): Promise<Response> {
    ...
    try {
        const { name, expiresAt } = await request.json() as { name: string, expiresAt: string | null };

        if (!name || !name.trim()) {
            return respond(false, HttpStatus.BAD_REQUEST, 'Name is required.');
        }

        const users: ExtraUser[] = (await env.kv.get("extraUsers", { type: 'json' })) || [];

        const newUser: ExtraUser = {
            id: crypto.randomUUID(),
            name: name.trim(),
            uuid: crypto.randomUUID(),
            trPass: crypto.randomUUID().replace(/-/g, '').slice(0, 16),
            expiresAt: expiresAt || null,
            createdAt: new Date().toISOString()
        };

        users.push(newUser);
        await env.kv.put("extraUsers", JSON.stringify(users));
        return respond(true, HttpStatus.OK, '', users);
    } catch (error) {
        console.log(error);
        return respond(false, HttpStatus.INTERNAL_SERVER_ERROR, `Error occurred while adding user: ${safeErrorMessage(error)}`);
    }
}
...
async function deleteUser(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        await env.kv.put("extraUsers", JSON.stringify(updatedUsers));
        return respond(true, HttpStatus.OK, '', updatedUsers);
    } catch (error) {
        console.log(error);
        return respond(false, HttpStatus.INTERNAL_SERVER_ERROR, `Error occurred while deleting user: ${safeErrorMessage(error)}`);
    }
}
